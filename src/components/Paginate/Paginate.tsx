...
export default function Paginate({ totalPages, queryConfig }: Props) {
  const location = useLocation() // Lấy current location
  let doAfter = false
  let doBefore = false
  ...
  const pagiNation = () => {
    ...
    return Array(totalPages)
      ...
      .map((_, index) => {
        ...
        if (page <= RANGGE * 2 + 1 && pageNumber > page + RANGGE && pageNumber < totalPages - RANGGE + 1) {
          return renderDoAfter(index)
        } else if (page > RANGGE * 2 + 1 && page < totalPages - RANGGE * 2) {
          if (pageNumber < page - 2 && pageNumber > RANGGE) {
            return renderDoBefor(index)
          } else if (pageNumber > page + 2 && pageNumber < totalPages - RANGGE + 1) {
            return renderDoAfter(index)
          }
        } else if (page >= totalPages - RANGGE * 2 && pageNumber > RANGGE && pageNumber < page - RANGGE) {
          return renderDoBefor(index)
        }
